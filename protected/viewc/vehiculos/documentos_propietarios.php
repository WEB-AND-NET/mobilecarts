        ...
        <div class="box-body">
            <fieldset style="width:97%;">
                <legend>Información General
                    <div class="box-footer col-lg-3 pull-right">
                        <?php if($data['rol']=="3"){ ?>
                            <a type="button" disabled href="<?= $patch ?>vehiculos_propietario/updatedocts/<?php echo $data['id'] ?>" id="btn-solicitar-r" class="btn bg-blue right">Solicitar Revision</a>
                            
                        <?php } else { ?>  
                            <a type="button" href="<?= $patch ?>vehiculos" class="btn bg-blue pull-right">Volver</a>  
                        <?php } ?>
                        </div>
                </legend>
                <?php foreach($data["documentos"] as $documento){ ?>
                   <form id="f<?= $documento->id ?>" data-id="<?= $documento->id ?>" style="padding:20px;" >
...
$(document).ready(function(e){
        var url = "<?php  echo $patch; ?>vehiculos_propietario/documents/getDocuments";
...
$("input[class=form-control]").on('blur',function (e) {
            ...
            var datos = "id_documento="+idDoc+"&nombre_documento="+nombreDoc+"&id_vehiculo=<?php echo $data['id'] ?>&"+$(this.parentElement.parentElement.parentElement.parentElement).serialize();
            savedoc(datos)
    ...
    function savedoc(datos){
        $.ajax({
            url: "<?php  echo $patch; ?>vehiculos_propietario/documents/save",
            type: 'post',
            data: datos,
            success: function(e){
                console.log(e);
    ...
    $(".doc").change(function(e){
        ...
        var datos = "id_documento="+$(this).attr("doc-id")+"&nombre_documento="+$(this).attr("attr-folder")+"&id_vehiculo=<?php echo $data['id'] ?>";
        savedoc(datos)
        
        $.ajax({
            url: "<?= $data["rootUrl"] ?>vehiculos_propietario/documents/saveAll",
            type: "post",
            dataType: "html",
            data:formData,
            cache: false,
