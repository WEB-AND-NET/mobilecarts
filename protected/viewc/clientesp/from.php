...
<br/>
<div class="box ">
    <form id="form1" class="form" action="<?= $patch; ?>clientesp/save" method="post" name="form1">
        <div class="box-body">
                    ...
                    <div class="input-group">
                        ...
                        <select class="form-control select2" id="id_propietario" name="id_propietario">
                            ...
                            <?php foreach ($data["propietarios"] as $m) { ?>
                                <option <?= ($a->id_usuario == $m->id ? 'selected="selected"' : ''); ?> value="<?= $m->id; ?>"><?= $m->razon_social; ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function validar() {
        $("#form1").mask("Espere...");
        $.post('<?= $patch; ?>clientesp/validar', 
            {
                id_propietario: $('#id_propietario').val(), ced: $('#identificacion').val(), id: $("#id").val()
            },
            function (data) {
                $("#form1").unmask();
                if (data) {
                    alert('EL Cliente se encuentra Registrado con este Propietario');
                } else {
                    $('#form1').submit();
                }
            }
