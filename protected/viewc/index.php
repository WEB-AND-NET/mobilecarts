...
<html lang="es" manifest="/msgaitan/msgaitan.appcache">
    ...
    <body class="skin-green sidebar-mini">
        <div class="wrapper">
            <?php $rol = $_SESSION['login']->role; $tipo = $_SESSION['login']->tipo;?>
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form role="form">
                                <div class="form-group col-lg-6">
                                    <label for="asg_cliente"><span class="glyphicon glyphicon-user"></span>&nbsp;Cliente</label>
                                    <input type="text" class="form-control" id="asg_cliente" >
                                </div>
                                <div class="form-group col-lg-12">
                                    <label for="asg_direccion"><span class="fa fa-location-arrow"></span>&nbsp;Direcci&oacute;n </label>
                                ...
                                <div class="clearfix"></div>
                                <div class="form-group col-lg-6">
                                    <label for="asg_clase_vehiculo"><span class="fa fa-car"></span>&nbsp;Tipo vehiculo</label>
                                ...
                                <div class="form-group col-lg-6">
                                    ...
                                    <br/>
                                    <select class="form-control select2" id="asg_id_conductor" onchange="cargarConductor(event);">
                                ...
                                <div class="clearfix"></div>
                                <div class="form-group col-lg-6">
                                    <label for="asg_identificacion"><span class="fa fa-sort-numeric-desc"></span>&nbsp;Identificaci&oacute;n</label>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-default" data-dismiss="modal" id="btn_cls_asignar_orden">Cerrar</button>
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form role="form">
                                <div class="form-group col-lg-6">
                                    <label for="can_cliente"><span class="glyphicon glyphicon-user"></span>&nbsp;Cliente</label>
                                    <input disabled="disabled" type="text" class="form-control" id="can_cliente" >
                                </div>
                                <div class="form-group col-lg-12">
                                    <label for="can_direccion"><span class="fa fa-location-arrow"></span>&nbsp;Direcci&oacute;n </label>
                                ...
                                <div class="clearfix"></div>
                                <div class="form-group col-lg-6">
                                    <label for="can_clase_vehiculo"><span class="fa fa-car"></span>&nbsp;Tipo vehiculo</label>
                                    <input disabled="disabled" type="text" class="form-control" id="can_clase_vehiculo">
                                </div>
                                <div class="clearfix"></div>
                                <div class="form-group col-lg-6">
                                    <label for="can_identificacion"><span class="fa fa-sort-numeric-desc"></span>&nbsp;Identificaci&oacute;n</label>
                        ...
                        <div class="modal-footer">                            
                            ...
                            <button type="button" class="btn btn-default" data-dismiss="modal" id="btn_cls_cancelar_orden">Cerrar</button>
  ...
  function loadnotify() {
    ...
    $.post(url,function(res){
        ...
        if(data.vencidos.length > 0){
            $("#footer-vencido").html("<a href='<?php echo $patch ?>getdocvencidos/view'>Ver todos los documentos vencidos</a>")
        }
        ...
        for(var i = 0; i < data.nuevoc.length; i++ ){
            ...
            dathtml1 += "<li><a href='<?php echo $patch ?>conductoresp/openotify/"+data.nuevoc[i].id+"' style='cursor:pointer; white-space: normal' id="+data.nuevoc[i].id+">"+data.nuevoc[i].mensaje+"<br><p style='width:80%; display:block; text-align:right'>"+data.nuevoc[i].fecha+"</p></a></li>";
        }
        if(data.nuevoc.length > 0){
            $("#footer-conductores").html("<a href='<?php echo $patch ?>conductoresp'>Ver todos los Conductores Inactivos</a>")
        }
    ...
    function cargar_asignar(id_orden){
        ...
        $.post('<?= $patch ?>ordenes_servicios/cargar_asignar',{id_orden : id_orden},function(data){
            console.log(id_orden);
            console.log(data);
            var o = data.orden;
    ...
    function cargarConductor(event){                
        $.post('<?= $patch?>ordenes_servicios/cargar_conductor',{id_con : event.target.value,id_veh :$("#"+event.target.id+" option:selected").attr("data_idv")},function(data){
        ...
        function initializeNotifications() {
            $.getJSON("<?= $patch ?>ordenes/pendientes", function (data) {
                ...
                if (data.ordenes.length > 0) {
                    ...
                    $("#push-ordenes-h").html("Upps! No hay Solicitudes.");
                var item = '';
                $.each(data.ordenes, function (key, val) {                    
                    item += '<li id="lipush'+val.id+'">' +
                            '<a onclick="myFunction(' + val.id + ')" id=' + val.id + ' href="javascript:void(0)">' +
                            '<div class="pull-left">'+
                    ...
                    switch(val.tipo){
                        case 'T':
                            item += '<p><strong>Origen: </strong> '+val.barrio_o+' &nbsp;-&nbsp;&nbsp; <strong>Destino:</strong> '+val.barrio_d+'</p>';
                        break;
                        case 'D':
        ...
        function updateNotifications() {
            ...
            $.getJSON(url, function (data) {
                if (data.ordenes.length > 0) {
                    ...
                    $("#push-ordenes-h").html("Tienes " + (data.ordenes.length + count) + " solicitudes pendientes.");
                    $.growl.notice({title: '<?= $login->nombre ?>', message: "Tienes " + (data.ordenes.length) + " solicitud(es) nueva(s).", location: "tr"});
                    count += data.ordenes.length;
                    window.document.title =  "<?= C_TITLE; ?> ("+count+")";
...
<script type="text/javascript">
    ...
    function cargar_cancelar(id_orden){
        $.post('<?= $patch ?>ordenes_servicios/cargar_cancelar',{id_orden : id_orden},function(data){
            console.log(data);
            var o = data.orden;
        ...
        function initializeNotifications2() {
            $.getJSON("<?= $patch ?>ordenes/cancelados", function (data) {
                ...
                if (data.ordenes.length > 0) {
                    ...
                    date2 = data.fecha;
                    $("#push-cancelados-h").html("Tienes " + data.ordenes.length + " solicitudes canceladas.");  
                } else
                    $("#push-cancelados-h").html("Upps! No hay Cancelaciones.");
                var item = '';
                $.each(data.ordenes, function (key, val) {
                    item += '<li id="lipush' + val.id + '">' +
                            '<a onclick="myFunction2(' + val.id + ')" id=' + val.id + ' href="javascript:void(0)">' +
                            '<div class="pull-left">' +
                            '<img src="<?= $patch ?>global/img/users/<?= $login->imagen ?>" class="img-circle" alt="User Image" />' +
                            '</div>' +
                            '<h4>' +
        ...
        function updateNotifications2() {
            ...
            $.getJSON(url, function (data) {
                if (data.ordenes.length > 0) {
                    ...
                    $("#push-cancelados-h").html("Tienes " + (data.ordenes.length + count2) + " solicitudes canceladas.");
                    $.growl.notice({title: '<?= $login->nombre ?>', message: "Tienes " + (data.ordenes.length) + " solicitud(es) cancelada(s).", location: "tr"});
                    count2 += data.ordenes.length;
