...
<div class="box col-md-10">
    <form id="form1" class="form" method="post" action="<?= $patch; ?>ing_vehiculo/html"  name="form1"><!-- target="_blank" -->
        <div class="box-body">
            <fieldset style="width:97%;">
                <legend>Informaci&oacute;n General</legend>
                <div class="col-lg-4">
                    <label id="l_id_vehiculo">Vehiculo</label>
                    <div class="input-group">
                        ...
                        <select class="form-control select2"  id="id_vehiculo" name="id_vehiculo">
                            ...
                            <?php foreach ($data["vehiculos"] as $v) { ?>
                                <option value="<?= $v->id; ?>" dataone="<?= $v->placa ?>"><?= $v->placa; ?></option>
                            <?php } ?>
                        </select>
                    </div><!-- /.input group -->
                </div>
...
<script type="text/javascript">
    ...
    $('#btn-save').click(function (e) {
        ...
        if (validateForm()) {
            var action = $('#form1').attr("action") + "/" + $('#id_vehiculo').val()+ "/" + $('#fecha_inicial').val()+ "/" + $('#fecha_final').val();
