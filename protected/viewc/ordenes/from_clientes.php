    ...
    <form id="form1" class="form" action="<?= $patch; ?>ordenes_servicios/save" method="post" name="form1">
        <div class="box-body">
            <fieldset style="width:97%;">
                ...
                <div class="col-lg-4" id="div_rutas" style="display: block;">
                    <label id="l_rutas">Ruta</label>
                    <div class="input-group">
                        <div class="input-group-addon">
                            <i class="fa fa-location-arrow"></i>
                        </div>
                        <select class="form-control select2" id="rutas" name="rutas" class="select">
                            <option value="">[Seleccione..]</option>
                            <?php foreach ($data["rutas"] as $r) { ?>
                                <option value="" data-origen="<?= $r["id_o"]; ?>" data-destino="<?= $r["id_d"]; ?>"><?= $r["nombreo"] . " - " . $r["nombred"]; ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $('select#rutas').change(function() {
        console.log($("select#rutas option:selected").attr("data-origen"));
        console.log($("select#rutas option:selected").attr("data-destino"));
        $("#barrio_o").val($("select#rutas option:selected").attr("data-origen"));
        $("#barrio_d").val($("select#rutas option:selected").attr("data-destino"));
    });
    ...
    //        if (tipo === "J") {
    //            $.post('<? //= $patch 
                            ?>ordenes_servicios/cargar_contacto', {id_client: id_client, id_contact: id_contact}, function (data) {
    ...
    $('#id_cliente').ready(configCliente);
        ...
        if (validateForm()) {
            ...
            $.post('<?= $patch; ?>ordenes_servicios/getitems2', {
                ...
                function(data) {
                    $("#form1").unmask();
                    $('#items2').html(data);
                    $("#sobre_tasa").val($("#sobre_tasa_edit").val());
                    calcularTotal();
                }
            );
        }
    ...
    function delItem2(i) {
        $("#form1").mask("Espere...");
        $.post('<?= $patch; ?>ordenes_servicios/delete2', {
            index: i
        }, function(data) {
            $("#form1").unmask();
            $('#items2').html(data);
    ...
    function validateForm() {
        ...
        if ($('#tipo_cliente').val() === "J") {
            sErrMsg += ($('#id_contacto option:selected').val() === "0" ? '- Debe seleccionar Un Contacto.\n' : '');
        }
