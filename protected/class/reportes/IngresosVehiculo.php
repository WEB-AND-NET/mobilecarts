 ...
 */
class IngresosVehiculo extends FPDF {

    public function __construct() {
        parent::__construct('L', 'mm', 'a3');
        $this->SetTitle('Ingresos por Vehiculo');
    }
    ...
    function Body($data) {
        ...
        $this->Cell(335, 5, utf8_decode('INGRESOS DE VEHICULO'), 0, 1, 'R');
        $this->Cell(66, 5, utf8_decode("NIT ".C_NIT_DIGITO), 0, 0, 'L');
        $this->Cell(335, 5, utf8_decode($vehiculo["propietario"]), 0, 1, 'R');
        $this->Cell(66, 5, "", 0, 0, 'L');
        $this->Cell(335, 5,$vehiculo["marca"]." ".$vehiculo["placa"], 0, 1, 'R');
        $this->Cell(66, 5, "", 0, 0, 'L');
        ...
        if(count($servicios) < 1){
            ...
            $this->Cell(40, 5, utf8_decode('CLIENTE'), 1, 0, 'C');
            $this->Cell(35, 5, utf8_decode('CENTRO DE COSTO'), 1, 0, 'C');
            $this->Cell(35, 5, utf8_decode('RESPONSABLE'), 1, 1, 'C');
            ...
            foreach ($servicios as $t) {
                ...
                $this->Row(array(
                    ...
                    utf8_decode($paradas),
                    utf8_decode($t["cliente"]),utf8_decode($t["centrocosto"]),utf8_decode($t["contacto"])
                ));    
            }
